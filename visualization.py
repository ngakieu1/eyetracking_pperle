...
prev_tvec = None  # Biến toàn cục để lưu giá trị tvec trước đó
...
face_model_all *= 10
landmarks_ids = [2, 30, 60, 45, 23, 15, 1]  # reye, leye, mouth
face_model = np.asarray([face_model_all[i] for i in landmarks_ids])

print("face_model_all shape:", np.shape(face_model_all))  # Check its size
print("landmarks_ids:", landmarks_ids)  # Check requested indices
...
def get_face_landmarks_in_ccs(camera_matrix, dist_coefficients, shape, results):
    ...
    if success:
        rvec, tvec = cv2.solvePnPRefineVVS(
            face_model, face_landmarks, camera_matrix, dist_coefficients, rvec, tvec
    )
    for _ in range(10):
        ...
        if success:
            # Chuyển đổi rvec thành ma trận xoay
            rotation_matrix, _ = cv2.Rodrigues(rvec)
            if rotation_matrix.shape == (3, 3):  # Kiểm tra kích thước ma trận
                rotation_matrix = np.hstack([rotation_matrix, np.zeros((3, 1))])  
            # Tách các góc Euler (pitch, yaw, roll) từ ma trận xoay
            pitch, yaw, roll = cv2.decomposeProjectionMatrix(rotation_matrix)[:3]

            # In giá trị yaw (xoay trái/phải)
            print(f"Yaw (xoay trái/phải): {yaw.flatten()[0]:.2f} độ")
    head_rotation_matrix, _ = cv2.Rodrigues(rvec.reshape(-1))

    global prev_tvec  # Dùng biến toàn cục

# Áp dụng bộ lọc làm mượt
    alpha = 0.9  # Hệ số làm mượt (0.9 -> giữ nhiều giá trị trước đó, 0.5 -> mượt mạnh hơn)
    if prev_tvec is not None:
        smoothed_tvec = alpha * prev_tvec + (1 - alpha) * tvec
    else:
        smoothed_tvec = tvec  # Không làm mượt nếu đây là khung hình đầu tiên

    prev_tvec = smoothed_tvec  # Cập nhật giá trị trước đó
...
def main(base_path: str, screen_height_mm_offset: int = 10):
    ...
    for idx, row in df.iterrows():
        ...
        # Nhấn phím 'q' để thoát vòng lặp sớm
...
if __name__ == '__main__':
    ...
    parser.add_argument("--base_path", type=str, required=True, default='./data/p00')
    args = parser.parse_args()

    main(args.base_path)
    print("Base path received:", args.base_path)

# chay: python visualization.py --base_path=./data/p00