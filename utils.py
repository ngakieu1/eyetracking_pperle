...
from pperlewebcam import WebcamSource
...
def show_point_on_screen(window_name: str, base_path: str, monitor_pixels: Tuple[int, int], source: WebcamSource) -> Tuple[str, Tuple[int, int], float]:
    ...
    key_mapping = {
        0: 82,  # MacOS 'Up' -> Standard OpenCV 'Up' (82)
        1: 84,  # MacOS 'Down' -> Standard OpenCV 'Down' (84)
        2: 81,  # MacOS 'Left' -> Standard OpenCV 'Left' (81)
        3: 83   # MacOS 'Right' -> Standard OpenCV 'Right' (83)
    }
    ...
    if end_animation_loop:
        ...
        while time.time() - start_time_color_change < 0.5:
            print(f"Waiting for key press... Expected key: {orientation.value}")
            key = cv2.waitKey(1000) & 0xFF  # Đọc phím nhấn từ bàn phím
            mapped_key = key_mapping.get(key, key)
            print(f"Key pressed (raw): {key}, Mapped key: {mapped_key}")   # In giá trị phím nhận được

            if mapped_key == orientation.value:
                ...
                print(f"Key matched! Image saved: {file_name}.jpg, Time: {time_till_capture}")
            else:
